...
//Es un contador independiente, proporciona un reloj/calendario
...
void RTC_Config(RTC_Handler_t *ptrRtcHandler) {
	...
	RTC->DR |= ((ptrRtcHandler->RTC_Config.RTC_Day) / 10) << RTC_DR_DT_Pos;   // Date Tens in BCD
	RTC->DR |= ((ptrRtcHandler->RTC_Config.RTC_Day) % 10) << RTC_DR_DU_Pos;   // Date Units in BCD


	RTC->CR |= ((ptrRtcHandler->RTC_Config.RTC_Format) << RTC_CR_FMT_Pos);  	// 0: 24h format, 1: 12h format
	...
	RTC->TR |= ((ptrRtcHandler->RTC_Config.RTC_Notation) << RTC_TR_PM_Pos);  // 0: is am or 24hformat 1: is pm
