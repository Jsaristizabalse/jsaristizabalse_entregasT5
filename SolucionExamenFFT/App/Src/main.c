...
#define FFT_SIZE    1024   			//Tamaño del arrlego de datos
...
float32_t acelSignal[FFT_SIZE];
float32_t transformedSignal[FFT_SIZE];
float32_t FFTFinal[FFT_SIZE];
float32_t max = 0;
uint16_t indexFFT = 0;
float32_t freqFFT = 0;
...
void parseCommands(char  *ptrbufferReception){
	...
	if(strcmp(cmd, "help") == 0){
		...
		writeMsg(&usart6Comm, "9) Encender FFT \"FFTEN @\" \n");
		writeMsg(&usart6Comm, "8) Empezar FFT \"FFT @\" \n");
	}
...
//----------------------------Fin de comandos psc
		...
		for (int i = 0; i < 512; i++) {
			...
			sprintf(bufferData, "Tomando los datos... \n");
			writeMsg(&usart6Comm, bufferData);

			int i = 0;
			int j = 0;
			sprintf(bufferData, "Iniciando FFT \n");
			writeMsg(&usart6Comm, bufferData);
			if(statusInitFFT == ARM_MATH_SUCCESS){
			...
			sprintf(bufferData, "el subindice de la frecuencia fundamental es %u \n", indexFFT);
			writeMsg(&usart6Comm, bufferData);
			freqFFT = (indexFFT*200/(fftSize));
			sprintf(bufferData, "La frecuencia es %#.4f Hz \n", freqFFT);
			writeMsg(&usart6Comm, bufferData);
		}
